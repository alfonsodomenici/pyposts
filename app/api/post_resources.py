...
def all():
    ...
    return response_with(resp.SUCCESS_200,{'posts':[post.to_json() for post in result]})
...
def create():
    ...
    db.session.add(post)
    db.session.commit()
    return response_with(resp.SUCCESS_201,{'post':post.to_json()})

@posts.route('/<int:id>')
@jwt_required()
def find(id):
    post=Post.find_by_id_secure(id,get_jwt())
    return response_with(resp.SUCCESS_200, {'post':post.to_json()})
